...
class WorkingAbility extends React.Component
{
	constructor(props)
	{
		...
		this.state = {
			dataSet : {
				...
				names : ['graduate', 'working-time', 'working-place', 'field-trip', 'working-online', 'field-work', 'transportation', 'sp-fluency', 'sp-os'],
	...
	render()
	{
		...
		for(i = 0; i < lim; i++)
		{
			returned.push(<RadioInputGroup values = {this.state.dataSet.values[i]} ids = {this.state.dataSet.ids[i]} name = {this.state.dataSet.names[i]} innerHtmls = {this.state.dataSet.innerHtmls[i]} content = {this.state.dataSet.contents[i]} onChange = {this.props.onChange} key = {this.state.dataSet.ids[i]}/>);
		}
		return(
			<>
			{returned}
